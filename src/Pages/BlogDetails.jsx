...
const BlogDetails = () => {
  ...
  if (!blog) {
    return (
      <div className="min-h-screen flex items-center justify-center font-bold text-4xl text-red-600 p-4">
        Blog not found 😥
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen ">
      {/* Header */}
      <section className=" px-2 text-center text-black"></section>
      ...
      <div className="py-6 px-4 sm:px-6 md:px-10 lg:px-16">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
          {/* Main Blog Content */}
          <article className="lg:col-span-3 bg-black p-4 sm:p-6 rounded-lg shadow-md">
            <img
              src={blog.image}
              alt={blog.title}
            ...
            <p className="text-gray-50 leading-relaxed whitespace-pre-line text-sm sm:text-base break-words">
              {blog.content}
            </p>
          </article>
          ...
          <aside className="space-y-3 sm:space-y-4 mt-10 lg:mt-0">
            ...
            {randomBlogs.map((b) => (
              <div
                ...
                className="flex items-center bg-black rounded-lg shadow-sm cursor-pointer hover:shadow-lg hover:scale-[1.02] transition p-2"
              >
                <img
                  src={b.image}
                  alt={b.title}
        ...
        <div className="max-w-7xl mx-auto mt-8 sm:mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {/* Latest Post */}
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-red-600 mb-3 sm:mb-4">
              Latest Post
            </h2>
            {hasLatest ? (
              <div
                ...
                className="flex flex-col sm:flex-row items-start sm:items-center bg-black rounded-lg shadow-md cursor-pointer hover:shadow-lg hover:scale-[1.03] transition p-4 sm:p-6"
              >
                <img
            ...
            {moreBlogs.length > 0 ? (
              moreBlogs.map((b) => (
                <div
                  key={b.id}
                  onClick={() => navigate(`/blog/${b.id}`, { state: b })}
                  className="flex items-center bg-black rounded-lg shadow-sm cursor-pointer hover:shadow-lg hover:scale-[1.02] transition p-2 mb-3 sm:mb-4"
                >
                  <img
                    src={b.image}
                    alt={b.title}
